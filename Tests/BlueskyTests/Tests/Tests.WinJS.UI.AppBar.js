		...
		return test.doAsync(function (onTestComplete) {
			...
			WinJS.UI.Pages.render('/Tests/supportFiles/appBar/basicAppbar.html', $testDiv[0]).then(function () {
				...
				$buttons.each(function (i, button) {
					...
					var $commandImage = $(">span.win-commandicon", $button);
					...
					// verify label
					var $label = $(">span.win-label", $button);
	...
	commandsProperty: function (test) {
		...
		return test.doAsync(function (onTestComplete) {
			...
			WinJS.UI.Pages.render('/Tests/supportFiles/appBar/complexAppbar.html', $testDiv[0]).then(function () {
				...
				var $b1 = $($("button", $appBar)[0]);
				var $b2 = $($("button", $appBar)[1]);
				var $b3 = $($("button", $appBar)[2]);
		...
		return test.doAsync(function (onTestComplete) {
			...
			return new WinJS.Promise(function (onComplete) {
				...
				return new WinJS.Promise(function (onComplete) {
					...
					appBar.onaftershow = function () {
						test.assert($testDiv.hasClass("win-appbar"), "2: Appbar not created");
						test.assert(appBar.placement == "bottom", "2: options not set");
						test.assert($testDiv.css("bottom") == "0px", "2: appbar top is incorrect");
						test.assert($testDiv.css("position") == "fixed", "2: appbar position is incorrect");
			...
			return new WinJS.Promise(function (onComplete) {
				...
				WinJS.UI.Pages.render('/Tests/supportFiles/appBar/customLayout.html', $testDiv[0]).then(function () {
					...
					appBar.onaftershow = function () {
						...
						test.assert($("button", $appBar).length == 8, "Did not create custom layout");
