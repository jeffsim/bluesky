...
testHarness.addTestFile("Windows.Storage Tests", {
    ...
    standardFolders: function (test) {
        ...
        return test.doAsync(function (onTestComplete) {
            ...
            test.assert(localFolder.folderRelativeId == "0\\0\\LocalState", "LocalFolder folderRelativeId incorrect");
            ...
            test.assert(tempFolder.folderRelativeId == "0\\0\\TempState", "tempFolder folderRelativeId incorrect");
            ...
            test.assert(roamingFolder.folderRelativeId == "0\\0\\RoamingState", "roamingFolder folderRelativeId incorrect");
            ...
            test.assert(appFolder.folderRelativeId == "0\\0\\AppX", "appFolder folderRelativeId incorrect");

            // Validate folder.path
            // TODO: How to validate front part of path?
            var curPackage = Windows.ApplicationModel.Package.current;
            test.assert(localFolder.path.indexOf("\\Local\\Packages\\" + curPackage.id.familyName + "\\LocalState") > 0, "Local Folder path incorrect");
            test.assert(tempFolder.path.indexOf("\\Local\\Packages\\" + curPackage.id.familyName + "\\TempState") > 0, "Temporary Folder path incorrect");
            test.assert(roamingFolder.path.indexOf("\\Local\\Packages\\" + curPackage.id.familyName + "\\RoamingState") > 0, "Roaming Folder path incorrect");
    ...
    msappProtocol: function (test) {
        ...
        return test.doAsync(function (onTestComplete) {
            // Apps can read from temp, romain, or local using ms-appdata or ms-appx protocols -- e.g.:
